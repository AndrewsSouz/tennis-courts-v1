...
import com.tenniscourts.stubs.TennisCourtStubs;
...
import static com.tenniscourts.stubs.ReservationStubs.*;
import static com.tenniscourts.stubs.ScheduleStubs.scheduleStub;
...
class ReservationServiceTests {
    ...
    @Test
    void BookAReservationShouldThrowException() {
        var createReservationDtoStub = createReservationRequestDTOStub();

        when(reservationRepository.existsByScheduleId(createReservationDtoStub.getScheduleId())).thenReturn(true);

        assertThrows(AlreadyExistsEntityException.class, () -> reservationService.bookReservation(createReservationDtoStub));
    }
    ...
    void cancelReservationShouldReturnTheReservation() {
        var schedule = Schedule.builder()
                ...
                .startDateTime(LocalDateTime.of(2022,5,1,15,0))
                .endDateTime(LocalDateTime.of(2022,5,1,16,0))
                .build();

        var reservationDtoStub = reservationDTOStub();
        var reservationStub = reservationStub();
        reservationStub.setSchedule(schedule);
